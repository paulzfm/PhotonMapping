...
void RayTracer::parseParams(const JsonBox::Value& val)
{
    ...
    Parser::checkOption(obj, CLS, "num_photons", Parser::INTEGER, "10000");
    ...
    if (obj.find("num_photons") != obj.end()) {
        _num_photons = Parser::asInteger(obj["num_photons"]);
    }
...
void RayTracer::buildPhotonMap()
{
    ...
    for (const auto& light : _scene->lights) {
        int n = light->area() * light->emittance * _num_photons / sum;
        for (int i = 0; i < n; ++i) {
    ...
    _photons_map->balance();
    printf("\t%d global photon emitted.\n", _photons_map->size());
}

void RayTracer::bounce(Photon& photon, int bounces)
{
...
void RayTracer::renderMap()
{
    ...
    int cnt = 0;
        ...
        for (int i = 0; i < width * height; i++) {
            ...
            if (record.hit) {
                RGB color = _photons_map->irradianceEstimate(record.v, _gathering_radius, record.n);
                _img->set(i, color);
            }
...
RGB RayTracer::pixelColor(const Ray& ray, int depth, double relevance)
{
    ...
    if (reflectance > 0) { // reflect
        reflect_color = roughness * _exposure
            * _photons_map->irradianceEstimate(record.v, 
                _gathering_radius, record.n) // diffuse
            + (1 - roughness)
